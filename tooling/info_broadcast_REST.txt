# THIS IS STAGED TO SIT AND REST, REOCCURINGLY BROADCASTING MESSAGES VIA A VANILLA API 
# INFO_BROADCAST IS MORE OF A QUICK AND FAST GREP IF YOU BADUSB HER
...
function _broadcastInfo() {
    ...
    # Displaying user information (entries separated by commas)
    echo -e "\n[*] Getting User List from /etc/passwd..."
    _user_list=$(awk -F: '{ print $1}' /#!/bin/bash
...
def start_restful_api(empireMenu, suppress=False, username=None, password=None, port=1337):
    ...
    def get_stagers_name(stager_name):
        ...
        if stager_name not in main.stagers.stagers:
            return make_response(jsonify({'error': 'stager name %s not foEmpireund, make sure to use [os]/[name] format, ie. windows/dll' %(stager_name)}), 404)
