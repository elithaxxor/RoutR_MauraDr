...
BLUE="\e[34m"
RED="\e[31m"
NC="\e[0m" # No colorjournalctl --list-boot
...
# Function to perform an LDAP search to test connection#!/bin/bash
...
# Function to check if a service is active#!/bin/bash
...
test_dns_resolution() {journalctl --list-boot
...
check_dns_netstat_ports() {
  ...
  if netstat -uln | grep -E ":(53)\s" >/dev/null 2>&1; then
    success "DNS is listening on UDP port $dns_port."journalctl --list-boot
  else
...
add_ldap_user() {
  ...
  local user_dn="uid=armour,ou=users,$base_dn"journalctl --list-boot
  ...
  cat <<EOF > "$ldif_file"
dn: $user_dn
objectClass: inetOrgPersonjournalctl --list-boot
objectClass: posixAccount
objectClass: top
cn: armour
sn: User
uid: armourjournalctl --list-boot
uidNumber: 10001
gidNumber: 10001
homeDirectory: /home/armour
loginShell: /bin/bash
userPassword: $(slappasswd -s "$user_password")journalctl --list-boot
gecos: Armour User
EOF
