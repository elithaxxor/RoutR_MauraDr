...
1. **User Authentication & Access**
   - Users log in via the frontend and receive a JWT token for secure API access.
   - All sensitive actions (scanning, exploiting, viewing results) require authentication.

2. **Network Scanning**
   - Users initiate a scan by entering a network CIDR and scan intensity.
   - The backend queues the scan task with Celery and runs it asynchronously.
   - Devices are discovered, fingerprinted (vendor, model, firmware), and results are returned.

3. **Payload Management**
   - The `payloads` directory contains exploit scripts/descriptions, each with metadata.
   - The Payload Manager indexes all payloads, allowing the backend to quickly find exploits relevant to each discovered device.

4. **Exploitation Workflow**
   - Users see a list of discovered devices in the frontend.
   - They can select a device and view all matching exploits, or choose "All" to run every applicable exploit.
   - The frontend sends the target(s) and payload choice to the backend.
   - The exploitation task is queued (Celery). For each target, the Payload Manager finds all matching payloads and executes each (currently mocked, but ready for real logic). Results are collected.

5. **Results & Reporting**
   - The frontend polls for task status/results.
   - Users see a detailed report of which exploits succeeded or failed on each device, with output/logs.
   - All actions are logged for security and traceability.

6. **Security & Quality**
   - All secrets/configs are managed via environment variables.
   - Rate limiting and input validation protect the API.
   - Linting and test scaffolding are in place for code quality.

### Typical User Flow
1. Login
2. Scan a network
3. View discovered devices
4. Select device(s) and exploit(s)
5. Run exploits (single or “All”)
